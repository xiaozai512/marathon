...
@IntegrationTest
class AppDeployWithReadinessWithAbdicationIntegrationTest extends AkkaIntegrationFunTest with MarathonClusterTest {
  ...
  test("deployment is restarted properly on master abdication") {
    Given("a new app with 2 instances and no readiness checks")
    ...
    When("updated with readiness checks and minimumHealthCapacity=1")
    ...
    And("first updated task becomes ready")
    ...
    waitForEvent("instance_changed_event")
    ...
    And("second updated task becomes ready")
